...
namespace Server.Mobiles
{
    public abstract class BaseGuildmaster : BaseVendor
    {
        ...
        public BaseGuildmaster(string title)
            : base(title)
        {
            this.Title = String.Format("o {0} {1}", title, this.Female ? "gra-mestra" : "grao-mestre"); 
            // here you'll set the "guildmaster" translation of your language.
            //Keep in mind that, if you have different translation both for female and male, the order remains (ex. this.Title = String.Format("the {0} {1}", title, this.Female ? "female" : "male");
            // on what you've put in each guildmaster class name. (ex.the bard guildmaster/guildmistress)
        }
